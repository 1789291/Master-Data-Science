...
# Tables and matrices -> otras estructuras de datos de R distintas.
# Suelen usarse para aplicar tests estadísticos, ya que muchas funciones estadísticas de R están hechas para trabajar
# con matrices o tablas

carrier_ratios.mat <- as.matrix(sqldf("SELECT Yes, No FROM carrier_ratios"))  # Convertimos a una matriz esas columnas
rownames(carrier_ratios.mat) <- sqldf("SELECT Carrier FROM carrier_ratios")[ , "Carrier"]  # A las matrices podemos darles rownames
carrier_ratios.mat
class(carrier_ratios.mat)

# Convertimos a una table
carrier_ratios.tbl <- as.table(carrier_ratios.mat) 
carrier_ratios.tbl
class(carrier_ratios.tbl)


( proportions <- round(prop.table(carrier_ratios.tbl, margin = 1),2) )  # la función prop.table me calcula una tabla de proporciones (porcentajes)
t(proportions)  # La transpongo
...
# Tesing all the airlnes at time (pero de dos en dos)
...
# Airlines 
test_result <- broom::tidy(test_result)  # Esto, con la librería broom, me pone bonito el resultado de los tests.
...
# Esto es simplemente para jugar y gestionar datos en una base de datos relacional
...
dbSendQuery(con, "SELECT * FROM flights WHERE Dest = 'JFK' LIMIT 10")  # Query sobre esa BD
...
# Esto tiene aplicación si trabajas con bases de datos de SQL, si no no mucha.


# Si cojo una query de dplyr y le pongo show_query() me lo traduce a SQL PROBAR.
