...
# Configuración de la sesión en Spark
...
# Le decimos dónde está el master de spark
# El master es un "servidor". En este ejemplo estará en local.
# Spark tiene un master y workers (los ordenadores del cluster). En este ejemplo sólo tenemos master
# Normalmente RStudio también estará en un servidor, y le daremos órdenes desde aquí.

sc <- spark_connect(master = "local",   # Esto peta por la versión de Java
                    version = "2.2.0",
                    config = conf)
...
flights_tbl <- copy_to(sc, flights, "flights", overwrite = T, memory = T)  # Envía un df local a spark
